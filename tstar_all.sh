...
cat ????????_all.txt | gawk '{if($4>0&&$4<'$tstar') print $9,$8,$4*2}' | gmt psxy -R -J -B -K -O -Scp -W0.5p,blue >> $PS
cat ????????_all.txt | gawk '{if($4<0&&$4>-'$tstar') print $9,$8,-$4*2}' | gmt psxy -R -J -B -K -O -S+p -W0.5p,red >> $PS
